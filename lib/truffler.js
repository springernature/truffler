...
Truffler.prototype._run = function(url, options, done) {
	...
	async.series({
		...
		openUrl: guardAgainstTimeout(function(next) {
			...
			state.page.onNavigationRequested = function(url, type, willNavigate, main) {
				console.log('====================');
			...
			state.page.onLoadStarted = function() {
				console.log('Now loading a new page...');
			};

			state.page.onLoadFinished = function(status) {
				console.log('Page load finished: ' + status);
			};

			state.page.onError = function(msg, trace) {
			...
			state.page.onResourceError = function(resourceError) {
			...
			state.page.open(url, function(error, status) {
				...
				if (status !== 'success') {
					return next(new Error('Error opening url "' + url + '" : ' + this.reason));
				}
